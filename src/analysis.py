...
class Analyzer(object):
    ...
    def __init__(self, language, n, phon_column, word_column, match_on, target, regressors):
        self.modes = ['neutral', 'anti_homophones', 'anti_homophones_plus']
    ...
    def extract_params(self, lexicon, formula=None):
        ...
        ### Skip rank stuff for now.

        ## Get rank neighborhood size, but normalized to maximum neighborhood size
        ### So max is 1, etc. (that way, intercept doesn't matter; it's just about decay rate)
        lexicon['neighborhood_normalized'] = lexicon['neighborhood_size'] / max(lexicon['neighborhood_size'])
        neighborhood_params = self.characterize_rank_distribution(lexicon, 'neighborhood_normalized')
    ...
    def extract_all_params(self):
        ...
        all_params.append(real_params)

        # Now get artificial lexica
        
        for i in self.artificial_lexica:
            fake_params = self.extract_params(i)
            fake_params['mode'] = i['mode'].values[0]
            all_params.append(fake_params)
